...
class ReActAgent(Agent):
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        while current_step < self.max_steps:
            ...
            if action.startswith("Finish"):
                ...
                if not final_answer:
                    # Finish 内容残缺（如修复截断后只剩 "["）：再要求模型完整重写，而不是接受空答案
                    try:
                        spinner = Spinner("Completing answer…")
                        spinner.start()
                        completed = self.llm.invoke(
                            [
                                {"role": "system", "content": (
                                    "You are finalizing an agent's answer. Rewrite and complete the final answer. "
                                    "Output only the answer content itself: no 'Finish' prefix, no brackets, no markdown headers."
                                )},
                                {"role": "user", "content": f"Task:\n{input_text}\n\nTruncated final-answer fragment:\n{action}"},
                            ],
                            max_tokens=800,
                        )
                        spinner.stop()
                        final_answer = (completed or "").strip("[] \t\n\r")
                    except Exception:
                        pass
                if not final_answer:
                    print("⚠️ Finish 内容为空，要求模型重新作答。")
                    self.current_history.append("Observation: 你的 Finish 内容残缺/为空。请改用规范格式 Finish[最终答案] 重新作答。")
                    continue
    ...
    def _parse_action_input(self, action_text: str) -> str:
        ...
        残缺内容（剥掉括号/空白后为空，如截断后只剩 "["）返回空串，由调用方重试补全。
        """

        def _clean(content: str) -> str:
            return content if (content or "").strip("[] \t\n\r") else ""
        ...
        if action_text.lower().startswith("finish"):
            return _clean(action_text[len("finish"):].strip())
