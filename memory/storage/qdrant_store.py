...
class QdrantVectorStore:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 嵌入式本地模式路径（免 Docker）：优先入参，其次环境变量
        self.local_path = kwargs.get("local_path") or os.getenv("QDRANT_LOCAL_PATH")
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            if self.url and self.api_key:
                ...
                # 嵌入式本地模式（纯 Python 落盘，免 Docker/服务；同一时刻仅单进程可访问）
                os.makedirs(self.local_path, exist_ok=True)
                self.client = QdrantClient(path=self.local_path)
                logger.info(f"✅ 使用 Qdrant 嵌入式本地存储: {self.local_path}")
            else:
                # 使用本地服务（默认）
                self.client = QdrantClient(
                    host="localhost",
