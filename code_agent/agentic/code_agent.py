...
from tools.builtin.arxiv_tool import ArxivTool
...
class CodeAgent:
    ...
    def __init__(self, repo_root: Path, llm: Optional[HelloAgentsLLM] = None, config: Optional[Config] = None):
        ...
        self.plan_tool = PlanTool(self.llm, prompt_path=str(self.paths.prompts_dir / "plan.md"))
        ...
        # 会话状态：当前主题库 + 当前模式（新工具据此读取默认 topic）
        self.session_state = SessionState(current_topic="default", mode=self.config.default_mode)

        # 科研工具：arxiv 检索/下载 + 论文 RAG 问答（均延迟导入重依赖）
        papers_root = self.config.get_papers_dir(self.paths.repo_root)
        qdrant_local = self.config.get_qdrant_local_path(self.paths.repo_root)
        self.arxiv_tool = ArxivTool(papers_root=str(papers_root), session_state=self.session_state)
        self.paper_rag_tool = PaperRagTool(
            collection_name=self.config.rag_collection,
            session_state=self.session_state,
            local_path=str(qdrant_local),
        )

        # 全量工具字典，供 ModeManager 按模式切分子注册表（共享同一批实例）
        self.all_tools = {
            "terminal": self.terminal_tool,
            "note": self.note_tool,
            "plan": self.plan_tool,
            "todo": self.todo_tool,
            "context_fetch": self.context_fetch_tool,
            "arxiv": self.arxiv_tool,
            "paper_rag": self.paper_rag_tool,
        }

        # 模式管理器：持有全量工具 + 每模式子注册表 + 每模式系统提示词
        ...
        # 中心化子 Agent 工具：需 mode_manager 构建受限子注册表，故在其后构造并回填 all_tools
        # （mode_manager.tools 与 all_tools 是同一对象引用，回填后子注册表即可见）
        ...
        self.all_tools["spawn_subagent"] = self.subagent_tool

        # 当前模式的工具注册表（ReAct 使用）
        self.registry = self.mode_manager.get_registry(self.session_state.mode)
        ...
        # 系统提示词随模式变化（paper/research/code），由 ModeManager 加载对应 mode_*.md
        self.system_prompt = self.mode_manager.get_prompt(self.session_state.mode)
        self.history: List[Message] = []
        self.recent_tool_packets: List[ContextPacket] = []
        self.last_direct_reply: bool = False

    def set_mode(self, mode: str) -> str:
        """切换模式：换系统提示词 + 工具子注册表（ReAct 每步重建工具描述，立即生效）。"""
        resolved = self.mode_manager.resolve(mode)
        self.session_state.mode = resolved
        self.system_prompt = self.mode_manager.get_prompt(resolved)
        self.registry = self.mode_manager.get_registry(resolved)
        self.react.tool_registry = self.registry
        label = MODES[resolved].label
        tools = ", ".join(self.mode_manager.tool_names(resolved))
        return (
            f"✅ 已切换到 /{resolved}（{label}）模式。当前主题库: {self.session_state.current_topic}\n"
            f"可用工具: {tools}"
        )
    ...
    def set_topic(self, topic: str) -> str:
        ...
        return f"✅ 当前主题库已切换为: {t}（/paper 问答与 arxiv 下载默认使用此库）"

    def current_mode(self) -> str:
        """返回当前模式与主题库的一行状态。"""
        label = MODES[self.session_state.mode].label
        return f"当前模式: /{self.session_state.mode}（{label}） | 主题库: {self.session_state.current_topic}"
    ...
    def run_turn(self, user_input: str) -> str:
        ...
        if self._is_chitchat(user_input):
            ...
            reply = "你好！我是 HelloAgents Academic Agent（科研辅助）。用 /paper 解读论文、/research 检索 arxiv、/code 部署或复现代码；/help 看全部命令。你想做什么？"
